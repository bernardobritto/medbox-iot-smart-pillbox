...
#include <MFRC522.h>        // Biblioteca para uso do módulo RFID MFRC522
...
// Definições de pinos para o módulo RFID, atuadores e buzzer
...
bool tagDetectada = false; // Flag para indicar se uma tag foi detectada

// Declaração de funções para o compilador C++
...
void setup() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    ESP.restart(); // Reinicia o ESP32 se não conseguir conectar ao WiFi
  }
  ...
  Serial.println("Inicializando MFRC522..."); // Correção: Faltava ); aqui
...
void loop() {
  ...
  if (mfrc522.PICC_IsNewCardPresent() && mfrc522.PICC_ReadCardSerial()) {
    ...
    int compartimento1, numAcionamento1;
    int compartimento2, numAcionamento2;
    int compartimento3, numAcionamento3;
  ...
  // Verifica se é hora de ativar o buzzer (por exemplo, às 20:28:00)
...
bool obterDadosDoCompartimento(const char* serverAddress, int &compartimento, int &numAtivamento) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    bool sucesso = false; // Flag para garantir que o http.end() seja sempre executado
    ...
    http.end(); // Correção: Agora é garantido que a conexão fecha, evitando travamentos do ESP32
    return sucesso;
  }
  return false;
}
...
void acionarAtuadoresSimultaneamente(int compartimento1, int numAtivamento1, int compartimento2, int numAtivamento2, int compartimento3, int numAtivamento3) {
  ...
  int maxAtivamentos = max(numAtivamento1, max(numAtivamento2, numAtivamento3));
  
  for (int i = 0; i < maxAtivamentos; i++) {
